...
async def group_filters(client, message):
    ...
    if 0 < len(message.text) < 100:    
        ...
        for i in "jaadugar".split() :
         if i in search.lower() :
            LuciferMoringstar=await message.reply_text(
                text=f"""<b>Hello 👋 {message.from_user.mention},</b>\n\nYou Are In Hurry Mood To ⬇️ Download The Movie <b>"Jaadugar"</b>\n\nBut This Movie Avilable Only In\n✔️ Film District Premium\n\n<b>For 💸 PREMIUM Membership,\n\n📥 Inbox To : [HeartBeat](t.me/helloheartbeat)</b>""",
            )
            await asyncio.sleep(60) # in seconds
            await LuciferMoringstar.delete()
            return
        for i in "sampurna".split() :
         if i in search.lower() :
            LuciferMoringstar=await message.reply_text(
                text=f"""<b>Hello 👋 {message.from_user.mention},</b>\n\nYou Are In Hurry Mood To ⬇️ Download The Series <b>"Sampurna"</b>\n\nBut This Series Avilable Only In\n✔️ Film District Premium\n\n<b>For 💸 PREMIUM Membership,\n\n📥 Inbox To : [HeartBeat](t.me/helloheartbeat)</b>""",
            )
            await asyncio.sleep(60) # in seconds
            await LuciferMoringstar.delete()
            return
